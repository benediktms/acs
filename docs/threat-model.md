...
| Self-registration identity confusion | `acs_register` accepts no session, binding identifier, or slug, requires supported host attestation, commits the new logical agent and binding atomically, and cannot replace an existing binding. Runtime reachability is checked separately for delivery. Active agent slugs are unique case-insensitively.                                                                                                | Operator-directed identity selection continues to use claims.                                                             |
