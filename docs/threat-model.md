...
| Self-registration identity confusion | `acs_register` accepts no session or binding identifier, requires supported host attestation and live-session inspection, commits the new logical agent and binding atomically, and cannot replace an existing binding. Active agent slugs are unique case-insensitively.                                                                                 | Any attested model may choose an unused logical-agent name; operator-directed identity selection continues to use claims. |
