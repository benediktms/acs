...
| Peer prompt elevation                | Peer content is rendered only as named tool output. ACS derives `principalKind` from the authenticated bearer principal and maps only bound agents to `workAuthority: delegated`; all deliveries retain `trustedForPermissions: false`. Content is never rendered as system, developer, user, configuration, or approval input.                           | Delegated work still runs only under the recipient's independently established local policy.                              |
