...
| Peer prompt elevation                | Peer content is rendered only as named tool output. ACS derives `principalKind` from the authenticated bearer principal and maps only bound agents to `workAuthority: delegated`. Content is never rendered as system, developer, user, configuration, or approval input.                                                                                                                                    | Delegated work still runs only under the recipient's independently established local policy.                              |
...
| Self-registration identity confusion | `acs_register` accepts no session or binding identifier, requires supported host attestation, commits the new logical agent and binding atomically, and cannot replace an existing binding. Runtime reachability is checked separately for delivery. Active agent slugs are unique case-insensitively.                                                                                                       | Any attested model may choose an unused logical-agent name; operator-directed identity selection continues to use claims. |
...
| Peer interruption                    | `priority`, message content, claims consumed by agents, and self-registration do not grant it. An operator grant is checked with the current sender binding and recipient opt-in immediately before one fenced exact-turn mutation; discovery is newest-first `thread/turns/list`, while `thread/read` only reconciles that exact ID. Stale or missing authority downgrades and ordinary delivery continues. | The shared Codex adapter remains disabled pending its isolated ownership matrix.                                          |
