...
- When the active-agent snapshot shows a peer who can act on a finding or own distinct work that materially advances the shared objective, proactively send one concise relevant message with `acs_send`. Do not wait to be asked, duplicate messages, or create unrelated coordination noise.
