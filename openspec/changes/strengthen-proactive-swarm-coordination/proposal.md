...
- Require user approval before creating a new managed worker for newly identified parallel work; use `acs codex workers create <agent>`, then wait for readiness acknowledgement before communication because creation receipts are submitted asynchronously and do not prove readiness; a later opportunity-driven inspection only refreshes coordination state.
