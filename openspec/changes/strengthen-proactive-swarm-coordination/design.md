...
- Give each started, resumed, or cleared managed session one complete paginated active-agent snapshot before it handles the user request.
...
3. Inject `STARTUP.md` through the existing managed SessionStart command. It keeps the required order deterministic: `acs_identity`, conditional `acs_register`, then one complete `acs_agents_list` cursor traversal before the user request. The repository hook reads the same canonical file from the Git root and suppresses itself when `ACS_MANAGED_SESSION_START=1`, so a managed repository session receives the guidance once.
...
- **Startup traverses a large agent catalog** -> Follow `nextCursor` only until absent to complete one bounded snapshot, then reuse it until a concrete coordination decision needs fresher state.
...
- **Two agents mutate one checkout while coordinating** -> Subscribe to `working`, input-required, and terminal events so acknowledgement is observable, then require an explicit ownership/worktree split before either performs branch, stash, or move operations.
