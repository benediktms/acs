...
- [x] 4.4 Complete the real-Codex reconnect and ambiguous-write matrix where feasible; verify a lost response after write leads to authoritative reconciliation or `acceptance-unknown`, never blind retry.
...
- [x] 4.9 Complete the live interactive matrix for locally-owned approval and user-input states; verify ACS defers without answering, denying, or bypassing the local request.
- [x] 4.6 Reconcile the implementation with https://github.com/benediktms/acs/pull/14, https://github.com/benediktms/acs/pull/15, https://github.com/benediktms/acs/pull/16, https://github.com/benediktms/acs/pull/19, and https://github.com/benediktms/acs/pull/35; verify each remaining PR either targets the direct-delivery contract or is explicitly superseded.
- [x] 4.7 Run strict OpenSpec validation plus affected test files, typecheck, lint, formatting, import-boundary, enum, and generated-protocol checks; leave the full suite to CI.
...
a local mock model endpoint; it does not certify real-model semantics. Live TUI
checks separately cover human ownership of approval and user-input prompts. Direct
delivery is enabled as the core path with per-attempt safety gates. The native CI
matrix validates both configured Codex versions; full-suite/TCK
results are reported by CI rather than inferred from compilation. PRs 14, 15, 16,
19, and 35 were closed unmerged as superseded by the direct-delivery, per-account
routing, or explicit task-lifecycle contracts.
